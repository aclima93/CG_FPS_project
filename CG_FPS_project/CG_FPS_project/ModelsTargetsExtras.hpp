...
void createTargetsAndExtras(){
    ...
    for(unsigned int i=0; i<targets.size(); i++){
        ...
        if( numSnow < numPerTarget){
            targets[i].Init( aux[pos][0], aux[pos][1], aux[pos][2], scaleX, scaleY, scaleZ, numBB, posBB, sizesBB, 0, 1, 1, 1, CAMO_SNOW_OBJ, CAMO_SNOW_TEXTURE);
            numSnow++;
        }
        else if( numCity < numPerTarget){
            targets[i].Init( aux[pos][0], aux[pos][1], aux[pos][2], scaleX, scaleY, scaleZ, numBB, posBB, sizesBB, 0, 1, 1, 1, CAMO_CITY_OBJ, CAMO_CITY_TEXTURE);
        }
        else{
            targets[i].Init( aux[pos][0], aux[pos][1], aux[pos][2], scaleX, scaleY, scaleZ, numBB, posBB, sizesBB, 0, 1, 1, 1, CAMO_JUNGLE_OBJ, CAMO_JUNGLE_TEXTURE);
            numJungle++;
        }
    ...
    for(unsigned int i=0; i<extras.size(); i++){
        ...
        extras[i].Init( aux2[pos][0], aux2[pos][1], aux2[pos][2], scaleY, scaleY, scaleZ, numBB2, posBB2, sizesBB2, 0, 1, 1, 1, lol1, lol2);
